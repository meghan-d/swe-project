  ...
  const validate = () => {
    ...
    formData.cards.forEach((card, index) => {
      if (card.cardNumber && !/^\d{16}$/.test(card.cardNumber)) {
        newErrors[`cardNumber${index}`] = "Not a valid card number (must be 16 digits)";
      }
    });
      ...
      <form onSubmit={handleSubmit} className="registration-form">
          ...
          <div key={index} className="card-section">
            <div className="form-group">
              ...
              <input
                ...
                className="input"
              />
              {errors[`cardNumber${index}`] && (
                <div className="error-message">{errors[`cardNumber${index}`]}</div>
              )}
            </div>
            ...
            {formData.cards.length > 1 && (
              <button
                ...
                onClick={() => handleRemovingCard(index)}
              >
                Remove Card
              </button>
            )}
        ...
        <div className="checkbox">
          ...
          <label htmlFor="promotions">
            {" "}
            Click here to register for promotional emails.
          </label>
        </div>
